...
- ✅ **DevTools "Replay Splash" action** — new `SplashReplayContext` owns the splash's visibility plus a `runId` remount key; `AppSplash` passes `runId` as the splash's React `key`. Pressing **Play** in DevTools re-runs the cameo → animation → fadeout sequence in place, so the retro GIF and MP4 variants are actually viewable — a reload was the only other way to see a splash, and it resets the in-memory flags to their defaults. The row's description names the variant `splashAnimation` currently resolves to.
