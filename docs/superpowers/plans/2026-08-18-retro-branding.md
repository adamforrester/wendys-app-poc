...
- **Lint: add no new problems.** `npm run lint` does **not** pass on this repo and never has — the baseline at commit `7d9140c` is **48 errors and 1 warning across 33 files**, in files unrelated to this feature (`gradients.ts`, `shadows.ts`, `Button.tsx`, `TextField.tsx`, `HomeScreen.tsx`, and others). So "lint passes" is not an achievable gate. Every file this feature touches — `vite.config.ts`, `featureFlags.ts`, `.storybook/preview.tsx`, `DeviceFrame.tsx`, `TopAppBar.tsx`, `BagButton.tsx`, `TopAppBar.stories.tsx`, `App.tsx`, `AccountScreen.tsx` — is currently **clean**, and must stay clean. Verify with `npx eslint <the files you changed>` and expect zero output. Do not fix unrelated pre-existing errors; that is out of scope.
